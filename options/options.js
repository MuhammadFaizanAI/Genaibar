...
const CMD_LABELS = {
  'screenshot-tab':    'Screenshot → paste into LLM',
...
async function loadAll() {
  ...
  llms = d.llms || []; activeLLMId = d.activeLLMId; prompts = d.prompts || [];
  renderPresets(); renderLLMList(); renderPromptList();
  const toggle = document.getElementById('floatingBtnToggle');
  toggle.checked = d.floatingBtnEnabled !== false;
}
...
function renderPresets() {
  ...
  PRESETS.forEach(p => {
    ...
    if (!added.has(p.id)) btn.addEventListener('click', () => addLLM(p));
    c.appendChild(btn);
  });
}
...
function renderLLMList() {
  ...
  if (!llms.length) { list.innerHTML = '<div class="list-empty">No LLMs yet. Use Quick Add or the form above.</div>'; return; }
  list.innerHTML = '';
  llms.forEach((llm, i) => {
...
async function addLLM(llm) {
  if (llms.find(l=>l.id===llm.id||l.url===llm.url)) { toast('Already added','error'); return; }
  llms.push(llm); if (!activeLLMId) activeLLMId = llm.id;
  await saveLLMs(); renderPresets(); renderLLMList(); toast(`${llm.name} added`,'success');
}
...
function renderPromptList() {
  ...
  if (!prompts.length) { list.innerHTML = '<div class="list-empty">No prompts saved yet.</div>'; return; }
  list.innerHTML = '';
  prompts.forEach((p, i) => {
    const row = document.createElement('div');
    row.className = 'list-row prompt-row';
...
async function checkShortcuts() {
  ...
  cmds.forEach(cmd => {
    ...
    if (cmd.shortcut) {
      ...
      k.innerHTML = '<span class="sc-warn">No shortcut — possible conflict</span>'; bad = true;
    }
    row.appendChild(n); row.appendChild(k); listEl.appendChild(row);
  });
  if (bad) {
    statusEl.innerHTML = `<span class="sw">Conflict detected.</span> <a class="slink" id="fixLink" href="#">Fix in Chrome →</a>`;
    ...
    statusEl.innerHTML = '<span class="sok">All shortcuts active.</span>';
  }
}
...
document.getElementById('floatingBtnToggle').addEventListener('change', async e => {
  await chrome.storage.sync.set({ floatingBtnEnabled: e.target.checked });
  toast(e.target.checked ? 'Floating button enabled' : 'Floating button disabled');
});
